...
export default ({
    ...
    await this.asignacionTiendaServicio.getEmpId(localStorage.usuario).then(data=>{
       ...
       //console.log(this.asigEmpId,"cps");
        this.viajesServicio.findByDia(dia).then(data=>{this.viaj=data.data;this.loading=false;
    ...
    await this.asignacionTiendaServicio.getEmpId(localStorage.usuario).then(data=>{
       this.asigEmpId=data.data;
       //console.log(this.asigEmpId,"cps");
        this.viajesServicio.findByDia(fecha).then(data=>{this.viaj=data.data;this.loading=false;
        for(let p=0;p<this.asigEmpId.length;p++){
            for(let r=0;r<this.viaj.length;r++){
                if(this.asigEmpId[p].tiendaSucursal===this.viaj[r].tienda){
                    y=y+1;
